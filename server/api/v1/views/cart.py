...
def add_to_cart():
    ...
    # if len(user_cart_exist.all()) != 0:
    #     return jsonify({"msg": "User's Cart is not Empty, Please Update"}), 402
    ...
    try:
        ...
        return jsonify({"message": "Items added to cart", "items": added_items}), 201
...
@app_views_bp.route("/cart", strict_slashes=False, methods=["DELETE"])
@jwt_required()
def delete_cart():
    user_identity = get_jwt_identity()
